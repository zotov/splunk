...
@Component
@Scope("prototype")
public class ReadRecManualSer extends ReadRec {
	
	private static final Logger log = LoggerFactory.getLogger(ReadRecManualSer.class);
	...
	public void readInvoice() {
		try {
			 ...
			 Thread t1 = new Thread(new Runnable(){
			      public void run() {
			    	  try {
			            int  invoiceCount = invoiceDaoManualImpl.getInvoiceCountSer(invoice.getClass());
			   		    log.info("read invoice manual ser count:{}",invoiceCount);
			          } catch (Exception ex) {
			    		 log.error("Error read ser, error message:{}",ex.getMessage());
			    	  }
			      }
	    	 });
			...
			log.error("Error ReadRecManualser ",ex);
		}
	}
