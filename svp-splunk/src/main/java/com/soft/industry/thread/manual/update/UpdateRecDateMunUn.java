...
@Component
@Scope("prototype")
public class UpdateRecDateMunUn extends UpdateRec {
	
	private static final Logger log = LoggerFactory.getLogger(UpdateRecDateMunUn.class);
	...
	public void updateInvoice() {
		...
		try {
		     Invoice invoiceChanged = invoiceDaoManualImpl.updateInvoiceUn(InvoiceDate.class, id,delay);
			 log.info("  id:{}, delay:{}, trhead:{}",id,delay,getComment(invoiceChanged));
		} catch(Exception ex) {
			log.error("Error update un , id:{}, delay:{},invoice:{}, erro-mess:{}", id, delay,invoice.getComment(), ex.getMessage());
		}
	}
