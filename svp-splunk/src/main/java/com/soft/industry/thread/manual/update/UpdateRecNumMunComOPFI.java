...
@Component
@Scope("prototype")
public class UpdateRecNumMunComOPFI extends UpdateRec {
	
	private static final Logger log = LoggerFactory.getLogger(UpdateRecNumMunComOPFI.class);
	...
	public void updateInvoice() {
		...
		try {
		     Invoice invoiceChanged = invoiceDaoManualImpl.updateInvoiceComOPFI(InvoiceNum.class, invoice.getId(),delay);
			 log.info(" trhead:{}, delay:{}",getComment(invoiceChanged),delay);
		} catch(Exception ex) {
			log.error("Error update com id:{}, invoice:{}, delay:{}, error mess:{}",id ,invoice.getComment() ,delay, ex.getMessage());
		}
	}
